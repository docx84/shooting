<template>
    <div class="hello">
        ...
        <action-button action="addPlayer" :payload="player" @success="success" @error="error">prova</action-button>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            success(player) {
                console.log(player)
                this.$toast.open('stored! (id:' + player.doc_id + ")")
            },
            error(message) {
                window.dump = message
                this.$toast.open({
